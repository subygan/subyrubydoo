    ...
    for line in lines:
        ...
        try:
            ...
            if not (line_strip in previous_state):
                create_habitica_task(update, context, line_strip)
        ...
        new_state[line_strip] = previous_state.get(line_strip, False)
        index += 1
...
def create_habitica_task(update: Update, context: CallbackContext, text):
    ic(text)
    try:
        user_id, api_key = get_habitica_user(update.message.from_user.id)
    except Exception as e:
        user_id, api_key = get_habitica_user(update.edited_message.from_user.id)
...
def complete_habitica_task(update: Update, context: CallbackContext, task_id):
